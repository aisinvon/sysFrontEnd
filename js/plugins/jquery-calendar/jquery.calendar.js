...
(function ($) {
    ...
    var curEid = $('#cur_eid').val();
    var curDate = parseInt($('#cur_date').val().replace(/-/g, ''))-1;
    ...
    var _private = {
                ...
                for (var i in data.settings.resources) {
                    if (data.settings.resources.hasOwnProperty(i)) {
                        if (iterator == index) {
                            return {
                                'id': i,
                                'name': data.settings.resources[i]
                            };
                        }
                        iterator++;
                    }
            ...
            if (data) {
                ...
                if (check.length > 1) {
                    ...
                    check.sort(function (a, b) {
                        return a.begins.getTime() - b.begins.getTime();
                    });
                    ...
                    for (var uid1 in check) {
                        ...
                        if (check.hasOwnProperty(uid1)) {
                            ...
                            for (var uid2 in check) {
                                ...
                                check.hasOwnProperty(uid2) && !(check[uid1].overlap.uid in check[uid2].overlap.items) && !(check[uid2].overlap.uid in check[uid1].overlap.items) &&
                                    (
            ...
            icalendar: function (ics) {
                ...
                $.each(lines, function (i, line) {
                    ...
                    case 'cal_event':
                        ...
                        // customied
                        case 'meetingRoomId':
                            event.meetingRoomId = matches[3];
                            break;
                        case 'dtstart':
                            event.begins = $[plugin_name].date(matches[3]);
                            break;
                        case 'dtend':
                            event.ends = $[plugin_name].date(matches[3]);
                            break;
                        case 'summary':
                            event.notes = matches[3];
                            break;
                            ...
                            //
                        case 'rrule':
                            event.repeats = parse._rrule(matches[3]);
                            break;
                        case 'exrule':
                            event.excepts = parse._rrule(matches[3]);
                            break;
                            ...
                            //
                        case 'rdate':
                            event.repeat.push($[plugin_name].date(matches[3]));
                            break;
                        case 'exdate':
                            event.except.push($[plugin_name].date(matches[3]));
                            break;
                        }
                        break;
                    }
        ...
        event: {
            ...
            update: function (bData, speed, ease) {
                ...
                if (data && values) {
                    ...
                    // if ('meetingRoomId' in bData) values.meetingRoomId = bData.meetingRoomId || '';
                    ...
                    var eachCaleTit = values.begins.format(data.settings.maskeventlabel) + '-' + values.ends.format(data.settings.maskeventlabel);
                    $event.find('p.title')[content_setter](eachCaleTit)/*.attr('title', eachCaleTit + '\n Requester: ' + values.notes)*/;
                    $event.attr('title', '<strong>Requester: </strong><br />' + values.title + '<br /><strong>Meeting Subject: </strong><br />' + values.notes);
            ...
            edit: function (e) {
                ...
                if (data && values) {
                    ...
                    }).on('mousedown.' + plugin_name, function (e) {
                        e.stopPropagation()
                    }).appendTo($event);
                    ...
                    if ($event.height() <= 30) {
                        ...
                        $textarea.css({
                            ...
                            height: 45,
                            opacity: 1
                        }, 'fast', data.settings.easing.eventeditin, function () {
                    ...
                    $textarea.blur(function () {
                        ...
                        if ($event.height() <= 30) {
                            ...
                            $(this).animate({
                                height: noteHeight,
                                opacity: 0
                            }, 125, data.settings.easing.eventeditout, function () {
                                $(this).remove();
                            });
                        } else {
                            $(this).remove();
        ...
        drag: {
            ...
            resizeB: function (e) {
                ...
                drag.deltaY = drag.lastY - e.pageY;
                drag.lastY = e.pageY;
                drag.incY += drag.deltaY;
            ...
            move: function (e) {
                ...
                // Calculate the new delta.
                drag.deltaX = drag.lastX - e.pageX;
                drag.lastX = e.pageX;
                drag.incX += drag.deltaX;
                drag.deltaY = drag.lastY - e.pageY;
                drag.lastY = e.pageY;
                drag.incY += drag.deltaY;
             ...
             */
                ...
                switch (e.data.type) {
                case 'resize':
                    data.settings.eventresize.apply(values.calendar, [values.uid, values, $event]);
                    break;
                case 'move':
                    data.settings.eventmove.apply(values.calendar, [values.uid, values, $event]);
                    break;
                }
            ...
            week: {
                ...
                cal: function (data) {
                    ...
                    for (var i = 0, mPast, mTime; i < data.cache.incrementsInDay; i++) {
                        ...
                        // clonedTimeLabel.attr('title', mTime);
                    ...
                    data.elements.dateline.add(data.elements.resourceline).css({
                        right: data.cache.scrollbarSize
                    });
                    ...
                    .append(data.elements.timeline)
                        ...
                        .append($("<div class='meet_cale_in_mask none'></div>"));
                ...
                event: function (data, values) {
                    ...
                    if (!('elems' in values) || values.elems.length < 1) {
                        ...
                        /**
                         * before edited
                         * only show remove btn when meeting is created by yourself, and the meeting will happen today and after
                         */
                        var valuesBegin = parseInt(values.begins.format('Y-m-d').replace(/-/g, ''));
                        if (values.eid === curEid && valuesBegin > curDate ){
                            $event.find('span.button-remove').show();                            
                        }else if(values.eid !== curEid){
                            $event.addClass('not_mine');
                        }
                        ...
                        $event.find('pre.details')[content_setter](values.title + '<br />' + values.notes);
                        ...
                        var eachCaleTit = values.begins.format(data.settings.maskeventlabel) + '-' + values.ends.format(data.settings.maskeventlabel),
                            mailLnk = "<a href='mailto:" + values.title + "'></a>";
                        
                        $event.find('p.title')[content_setter](eachCaleTit)/*.attr('title', eachCaleTit + '\n Requester: ' + values.title)*/;
                        // $event.attr('title', '<strong>Requester: </strong><br />' + values.title + '<br /><strong>Meeting Subject: </strong><br />' + values.notes).attr('meetingroomid', values.meetingRoomId);
                        $event.attr('title', 'Requester: \n' + values.title + '\nMeeting Subject: \n' + values.notes).attr('meetingroomid', values.meetingRoomId);
                        if(values.eid !== curEid){
                            $event.wrapInner(mailLnk);    
                        }
                ...
                position: function (speed, ease, detect) {
                    ...
                    if (data && values) {
                        ...
                        $events.each(function (i, event) {
                            ...
                            $event.find('p.title')[content_setter](eachCaleTit)/*.attr('title', values.notes + '\n Requester: ' + values.title)*/.attr('data-time-s', values.begins.format(data.settings.maskeventlabel)).attr('data-time-e', values.ends.format(data.settings.maskeventlabel));
                            // $event.attr('title', '<strong>Requester: </strong><br />' + values.title + '<br /><strong>Meeting Subject: </strong><br />' + values.notes);
                            $event.attr('title', 'Requester: \n' + values.title + '\nMeeting Subject: \n' + values.notes).attr('meetingroomid', values.meetingRoomId);
                            
                            // only show .meet_cale_in_mask when meeting is today before
                            /*var valuesBegin = parseInt(values.begins.format('Y-m-d').replace(/-/g, ''));
                            if(valuesBegin < (curDate + 1)){
                                $event.parent().siblings('.meet_cale_in_mask').show();
                            }else{
                                $event.parent().siblings('.meet_cale_in_mask').hide();
                            }*/
                ...
                cal: function (data) {
                    ...
                    for (var i = 0; i < data.settings.daystodraw; i++) {
                        ...
                        var clonedDate = data.elements.dayblock.clone(true)
                            ...
                            .find('p')[content_setter](clonedDateObject.getDate())
                            .end();
    ...
    var fragments = {
        ...
        event: $([
   '<div data-title="no_tit_tip" class="ui-' + plugin_name + '-event mid">',
    '<p class="resize-top" />',
    '<p class="title" />',
    '<span title="remove" class="button-remove none" />',
    '<pre class="details" />',
    '<p class="footer" />',
   ...
   '</div>'
        ...
        time: $('<div class="ui-' + plugin_name + '-time" title="" time="" past="" />'),
        labels: {
        ...
        init: function (options) {
            ...
            return $(this).each(function () {
                ...
                if (!data) {
                    ...
                    case const_month:
                        // Pass the data to save a call to the data API.
                    ...
                    case const_week:
                        ...
                        // Work out widths and heights.
                        data.cache.resourceWidth = data.settings.minwidth;
        ...
        add: function (bData) {
            ...
            if (data) {
                ...
                var values = {
                    ...
                    notes: bData.notes || '',
                    meetingRoomId: bData.meetingRoomId || null,
                    eid: bData.eid || null,
                    cache: {},
                    overlap: {
                        inset: [],
    ...
    $[plugin_name].colors = {
        ...
        HexToRGB: function (h, a) {
            return (a !== undefined ? 'rgba' : 'rgb') + '(' + $[plugin_name].colors.HexToR(h) + ', ' + $[plugin_name].colors.HexToG(h) + ', ' + $[plugin_name].colors.HexToB(h) + (a !== undefined ? ', ' + a + ')' : ')');
        },
        HexToR: function (h) {
            return parseInt(($[plugin_name].colors.cutHex(h)).substring(0, 2), 16)
        },
        HexToG: function (h) {
            return parseInt(($[plugin_name].colors.cutHex(h)).substring(2, 4), 16)
        },
        HexToB: function (h) {
            return parseInt(($[plugin_name].colors.cutHex(h)).substring(4, 6), 16)
        },
        cutHex: function (h) {
            return (h.charAt(0) == "#") ? $[plugin_name].colors.padHex(h.substring(1, 7)) : $[plugin_name].colors.padHex(h)
        },
        padHex: function (h) {
            return h.length > 3 ? h : h.substring(0, 1) + h.substring(0, 1) + h.substring(1, 2) + h.substring(1, 2) + h.substring(2, 3) + h.substring(2, 3);
        },
    ...
    var _replace = {
        ...
        D: function () {
            return $[plugin_name].lang[this.lang].short_day[this.getDay()];
        },
        j: function () {
            return this.getDate();
        },
        l: function () {
            return $[plugin_name].lang[this.lang].long_day[this.getDay()];
        },
        N: function () {
        ...
        // Month
        F: function () {
            return $[plugin_name].lang[this.lang].long_month[this.getMonth()];
        },
        m: function () {
            return (this.getMonth() + 1 < 10 ? '0' : '') + (this.getMonth() + 1);
        },
        M: function () {
            return $[plugin_name].lang[this.lang].short_month[this.getMonth()];
        },
        n: function () {
        ...
        P: function () {
            var str = this.toString().match(/GMT([+-][0-9]+) \(([a-zA-Z]+)\)/)[1].split('');
            str.splice(-2, 0, ':');
            return str.join('');
        },
        T: function () {
        ...
        // Full Date/Time
        c: function () {
            return _replace.Y.apply(this) + '-' + _replace.m.apply(this) + '-' + _replace.d.apply(this) + 'T' + _replace.H.apply(this) + ':' + _replace.i.apply(this) + ':' + _replace.s.apply(this) + _replace.P.apply(this);
        },
        C: function () {
            return _replace.c.apply(this).replace(_replace.P.apply(this), '') + 'Z';
        },
        r: function () {
    ...
    var _replaceUTC = {
        ...
        D: function () {
            return $[plugin_name].lang[this.lang].short_day[this.getUTCDay()];
        },
        j: function () {
            return this.getUTCDate();
        },
        l: function () {
            return $[plugin_name].lang[this.lang].long_day[this.getUTCDay()];
        },
        N: function () {
        ...
        // Month
        F: function () {
            return $[plugin_name].lang[this.lang].long_month[this.getUTCMonth()];
        },
        m: function () {
            return (this.getUTCMonth() + 1 < 10 ? '0' : '') + (this.getUTCMonth() + 1);
        },
        M: function () {
            return $[plugin_name].lang[this.lang].short_month[this.getUTCMonth()];
        },
        n: function () {
        ...
        // Full Date/Time
        c: function () {
            return _replaceUTC.Y.apply(this) + '-' + _replaceUTC.m.apply(this) + '-' + _replaceUTC.d.apply(this) + 'T' + _replaceUTC.H.apply(this) + ':' + _replaceUTC.i.apply(this) + ':' + _replaceUTC.s.apply(this) + _replaceUTC.P.apply(this);
        },
        C: function () {
            return _replaceUTC.c.apply(this).replace(_replaceUTC.P.apply(this), '') + 'Z';
        },
        r: _replace.r,
        U: function () {
    ...
    $[plugin_name].date = function (newdate, newtime, lang) {
        ...
        if (newdate instanceof Date) {
            ...
            if (newdate.toString() !== '') {
                ...
                if (split_date === null) {
                    var _d = new Date();
                    split_date = ['', _d.getFullYear(), _d.getMonth() + 1, _d.getDate()];
                }
        ...
        NewDate.getMonthsBetween = function (d, round) {
            ...
            var daysInTmp1 = tmp1.copy();
            daysInTmp1.setDate(1);
            daysInTmp1.setMonth(daysInTmp1.getMonth() + 1);
            daysInTmp1.setDate(0);
            daysInTmp1 = daysInTmp1.getDate();
            var daysInTmp2 = tmp2.copy();
            daysInTmp2.setDate(1);
            daysInTmp2.setMonth(daysInTmp2.getMonth() + 1);
            daysInTmp2.setDate(0);
            daysInTmp2 = daysInTmp2.getDate();
        ...
        NewDate.getWeeksBetween = function (d, round) {
            ...
            var tmp2 = d.copy();
            if (round) {
                tmp1 = tmp1.addDays(1 - tmp1.getDay());
        ...
        NewDate.incrementBy = function (value, amount) {
            ...
            if (increment !== 0) {
                ...
                case 'sec':
                case 'second':
                    return this.addMinutes(increment / 60);
                case 'min':
                case 'minute':
                    return this.addMinutes(increment);
                case 'hour':
                    return this.addHours(increment);
                case 'day':
                    return this.addDays(increment);
                case 'week':
                    return this.addDays(increment * 7);
                case 'month':
        ...
        NewDate.getIncrementBetween = function (date, value) {
            ...
            if (increment !== 0) {
                ...
                case 'sec':
                case 'second':
                    return ((this.getMinutesBetween(date) / 60) / increment);
                case 'min':
                case 'minute':
                    return (this.getMinutesBetween(date) / increment);
                case 'hour':
                    return (this.getHoursBetween(date) / increment);
                case 'day':
                    return (this.getDaysBetween(date) / increment);
                case 'week':
                    return ((this.getDaysBetween(date) / 7) / increment);
                case 'month':
                    return (this.getMonthsBetween(date) / increment);
                case 'year':
                    return (this.getYearsBetween(date) / increment);
                }
            }
        ...
        NewDate.roundToIncrement = function (value) {
            ...
            if (increment !== 0) {
                ...
                case 'sec':
                case 'second':
                    rounder = 1000 * increment;
                    break;
                case 'min':
                case 'minute':
                    rounder = 60000 * increment;
                    break;
                case 'hour':
                    rounder = 3600000 * increment;
                    break;
